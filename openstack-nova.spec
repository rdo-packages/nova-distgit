...
# For EL7 and EL8, we minimum supported versions that are tested by QE:
%if 0%{?rhel} == 7
...
%else
# In all other cases (including Fedora), stick to minimum versions that
# upstream Nova requires.  The below versions map onto
# MIN_{LIBVIRT,QEMU}_VERSION for 'Stein' release.  This way, it also
# won't break RDO's Fedora builders.
