...
# Note that _libdir is set to /usr/lib for noarch builds in CBS, so we need
# to use %{_prefix}/lib64/ instead which is a valid path for both x86_64 and ppc64.
Requires(pre):  %{_prefix}/lib64/qemu-kvm/hw-display-virtio-vga.so
%if 0%{?rhel} == 8
...
%changelog
* Wed Mar 30 2022 RDO <dev@lists.rdoproject.org> 1:25.0.0-1
- Update to 25.0.0
